...
- 文档入口为 `docs/README.md`；核心接手文档保持 README/CORE/PROGRESS/ISSUES/FILES/UI/GIT_PUBLISH，审计报告、发布说明和专项指南可按需保留。
...
- 线上 Linux 构建使用 `.github/workflows/build-harmonyos.yml`，当前固定生成 HAP-only；release 和 workflow artifact 只上传 `.hap`，不再生成 APP、`.app.zip`、`manifest.json` 或 `SHA256SUMS.txt`。
...
- **核心修改流程**：在 13 项目修改 → git push → GitHub Actions 在线构建并发布 latest release → 11 项目构建脚本自动下载 `librustdesk_core.a` → 放入 `entry/src/main/libs/arm64/`
...
- **当前兼容版本**: 1.4.7（由 `CoreBuildInfo.ets` 记录）
- **默认核心下载**：`https://github.com/liyan-lucky/librustdesk_core/releases/latest/download/librustdesk_core.a`
- **核心更新规则**：默认不固定 release tag 和 SHA；13 项目自动发布新 tag 后，只要该 release 成为 latest 且资产名仍为 `librustdesk_core.a`，11 项目构建会自动拉取新核心。如需固定版本，设置 `RUSTDESK_CORE_URL` 和 `RUSTDESK_CORE_SHA256`。

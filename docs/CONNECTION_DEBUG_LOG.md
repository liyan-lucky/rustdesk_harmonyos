...
- Cleanup follow-up: temporary screenshots/layout dumps and old report binaries were deleted after documenting the non-sensitive facts. Current HAP/Core artifacts are under `%VSCODE_ROOT%\99_Temp`, not the app repo `.codex_*` directories. The 2026-06-21 16:26 second cleanup also removed old worktree-root `_tmp_*`, old target/HAP/clone/log/cache directories, and IDE/tool caches; the authoritative retained/deleted list is `docs/DIRECTORY_CONVENTIONS.md`.

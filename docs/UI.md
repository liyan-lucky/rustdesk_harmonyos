...
- 共享密码菜单保存失败时保持打开，显示中文错误；成功时显示“共享设置已保存”。
- 永久密码仅显示是否已配置，不从核心读取或回显明文；一次性密码长度只接受官方支持的 6、8、10 位。
# 2026-08-18 登录同步入口

- 登录页标题栏、登录弹窗标题栏和账户用户列表标题栏增加同步图标；同步中显示加载状态并禁用重复点击。
- 页面优先显示缓存内容，手动同步成功显示中文提示，用户列表失败保留已有缓存内容。
- 细节调整：账户同步按钮位于连接页选项卡右侧、搜索按钮之后，与通讯录刷新共用 `refresh.svg`、18vp 尺寸、主题滤色和旋转动画；独立登录页保持相同的“搜索、同步”顺序和动效。
- 用户列表同步错误需区分 API 不可达、授权失效、无访问权限、服务端错误和响应格式异常，不能将所有失败都显示为网络故障。

## 2026-08-18 当前 UI 验收基线

- 连接页账户标签与独立登录页均在搜索按钮之后显示同步图标；同步期间旋转且禁止重复触发。
- 地址簿操作使用弹出菜单和独立的“增加 ID”“增加标签”对话框，不能共用布局或挤压设备列表。
- 标签区域完整显示服务器标签及颜色，支持创建、重命名、删除、分配、取消选择和按标签过滤；操作完成后当前页面必须立即刷新。
- 会话工具栏保持无背景线框容器，内部图标使用圆形/胶囊触控外观并适配亮暗主题。
- 自定义键盘按键透明度为 50%，背景区域负责屏蔽画面控制触摸，但不得阻止按键自身点击；系统键盘关闭时自定义键盘同步关闭。

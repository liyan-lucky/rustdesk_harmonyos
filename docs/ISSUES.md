...
**已尝试**: (1) module.json5 添加 `softInputMode: adjustResize` — 全屏模式下不生效; (2) WindowChromeService 监听 `avoidAreaChange TYPE_INPUT` — 不触发; (3) `window.on('keyboardHeightChange')` — 不触发; (4) TextInput onFocus/onBlur 手动设置 translate — translate 未生效。

...
### GitHub Release 直接上传 `.app` 资产失败

**现象**：HAP/APP 已生成，但 release 上传 `.app` 资产失败或不稳定。

**根因**：HarmonyOS `.app` 是目录/包格式，在 GitHub Release asset 上传链路中不适合作为裸 `.app` 直接上传。

**解决**：发布前将每个 `.app` 压缩为同名 `.app.zip`，release 只上传 `.hap`、`.app.zip`、`manifest.json`、`SHA256SUMS.txt`。

**教训**：✅ workflow 构建产物可以保留 `.app`，但 release 资产必须使用 `.app.zip`；✅ `SHA256SUMS.txt` 应在 zip 后生成，确保记录的是最终上传资产。
...
- ✅ 13 项目已构建并发布 `v1.4.7-ohos` core，11 项目 Linux CI 使用该 core 成功构建并发布 HAP/APP

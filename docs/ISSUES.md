...
## 2026-09-17 密码弹窗时机优化 — 连接前不弹窗

**现象**：点击连接设备时立即弹出密码输入框，但此时连接尚未建立，给用户"已连接成功"的错觉。

**根因**：`Index.ets:5515` `handleConnect` 无保存密码时主动调用 `showConnectPasswordDialog(targetId, true)` 立即弹窗，同时并行发起空密码后台连接。

**修复**：移除主动弹窗，改为先显示"连接中"状态 + 发起后台连接 + 启动监控循环。当对端真正要求密码时，`input-password` 事件（bridgeListener 261 行）或监控循环（monitorConnectionWhileWaiting 9462 行）自动弹出密码框。用户体验变为：点击连接 → 显示"连接中" → 对端要求密码 → 弹出密码框。
...
### 41. IME 自动补全右符号无法正确删除

**现象**：手机输入法输入 `（` 等左符号时自动补出右符号，本地光标停在符号中间；远端收到整对符号后光标却在右符号之后，Backspace 删除结果与手机端不一致。

**根因**：隐藏 TextInput 与远端输入框的光标位置发生分叉。仅比较前后文本无法修正光标语义，`isAutoCompletionDeletion` 只能减少错误删除次数，不能消除分叉。

**解决**：识别 IME 一次提交的常见成对符号，只向远端发送左符号，随后立即恢复隐藏 TextInput 的 sentinel 和光标。这样下一次 Backspace 会直接映射为远端对左符号的一次删除，不再保留无法操作的自动右符号。

### 40. 自建 API 第三方登录项未跟随服务器配置

**现象**：自建 API 的 `/api/login-options` 配置与登录页不一致；服务器未配置第三方登录时仍会出现官方或本地静态提供商。

**根因**：登录页底部“其他方式”绕过接口无条件读取本地提供商列表；接口解析也遗漏了官方支持的 `common-oidc/<JSON>` 格式。

**解决**：OAuth UI 统一只渲染接口结果，支持 `oidc/<name>` 和 `common-oidc/<JSON>`；仅使用官方 API 配置时允许官方默认集合兜底，自建 API 返回空或请求失败时保持空列表，不回退官方。

**补充根因**：项目同时存在独立 `LoginPage.ets` 和首页 `Index.ets` 内嵌登录弹窗。首次修复只覆盖独立页面，实际入口仍在首页硬编码 GitHub、Google、Microsoft。最终将提供商解析收敛到 `AccountService.getAvailableProviders()`，两个入口共用同一动态列表。

**GitHub ERR-2212 诊断**：2026-08-14 在手机端复现。`/api/oidc/auth` 能正常返回 GitHub 授权 URL，GitHub 能识别 OAuth 应用 `RustDesk-API-Pro-DIY`，随后自建 API 的回调页面返回 `ERR-2212`。该错误发生在服务端回调处理阶段（令牌交换、用户资料读取或账户绑定），不是 HarmonyOS 浏览器拉起或客户端提供商解析错误。应检查 API 服务端安全审计日志、GitHub OAuth Client Secret、回调配置和账户绑定策略；客户端不应为单个服务端错误注入未确认的强制授权参数。
...
**修复**：`3ebdc726` 移除隐藏 URL 的静默优先级；两套 workflow 默认 latest，并允许 dispatch 显式传入两架构 URL+SHA256。run `27920708116` 使用 `core-34` 精确 URL 与哈希重发，包内同时命中 `90A283...` / `E58746...`。审计新增 workflow 来源检查和 x86_64 CoreBuildInfo 强制断言。

**规则**：workflow 绿色、版本号相同、Release 名称正确都不足以验收；必须下载 HAP，验证签名/双 ABI/依赖，并从 `modules.abc` 或等价包内证据核对两架构大小与完整 SHA256。

- 已解决：ID 数字分组中途编辑光标跳尾、悬浮建议未完整写入、IP 卡片无法匹配；真机复验通过。
- 已解决：菜单过宽、可见“开发中”占位、管理员终端文案过长、重试按钮过高及中继文案过长；未实现项改为点击 toast。
- 已解决：共享页重复状态/错误文本、服务与录屏指示联动和刷新、文件传输/终端主题与滑动起始位置不统一。
- 已搁置而非缺陷：华为手机作为被控端的输入注入/accessibility。不要恢复虚假的可用开关，也不要把它列为发布阻塞。
- 最终固定哈希审计为 0 FAIL；若后续功能源码再改，必须重新构建、安装并生成新的完整证据链，旧哈希不得沿用。

## 2026-06-21 路径分散和临时文件污染
...
**现象**：真机共享画面已可被 Windows RustDesk 看到并持续刷新，但远端点击只产生 mouse event，native 注入返回 `result=201`，目标 UI 不变化。用户随后确认华为手机不支持本项目需要的被控端操控能力。

**边界**：Huawei 设置页未列出 OpenRDesk，第三方应用不能绕过系统授权自行启用 accessibility。2026-06-21 17:06 起，该能力从本轮 P0/发布阻塞项降级为 backlog。

**处理结果**：`entry/src/main/module.json5` 已移除 accessibility `extensionAbilities` 与 `ohos.permission.INPUT_MONITORING`，后续 HAP 不再把不可启用的 accessibility extension 作为发布能力。`CMakeLists.txt` 已移除 `ohinput` 链接和真实 input bridge 源文件，ArkTS accessibility service 已删除；仅由 `ohos_stubs.cpp` 保留 native/Core input injection 同名符号并固定返回 `201`，作为链接兼容和诊断边界；`result=201` 不再阻塞其他功能收口。

**后续恢复条件**：只有在目标系统明确提供第三方可用的输入注入或 accessibility 启用入口，并由用户手动授权后，才重新评估该功能。不得为通过验收伪装 profile 类型、绕过授权或把收到 input event 当作操控成功。

## 2026-06-20 显示远程光标只有 UI 外壳，Core 回调为空

**现象**：会话菜单可勾选“显示远程光标”，Core 也能记录 `show-remote-cursor=Y`，但画面上没有远端光标。

**根因**：App `RemoteControl.ets` 已准备 cursor position/data/PixelMap overlay；但 Core `rustdesk-master/src/harmony_bridge/core.rs` 的 `HarmonySessionHandler::set_cursor_data`、`set_cursor_id`、`set_cursor_position`、`set_display` 为空实现，`main_set_cursor_position` 也为空，官方 Session cursor 回调没有转成 bridge event。

**下轮要求**：优先沿用官方 `Interface` 回调命名，排入 cursor data/id/position/display 事件；验证热点坐标、缩放/旋转映射、多显示器和选项关闭。随后按同一标准检查所有会话菜单，日志下发不能替代真实效果。

## 2026-06-20 三点菜单阻止输入状态错误，取消时疑似断线

**真机现象**：启用“阻止用户输入”后功能可能已发送，但菜单项仍未选中；再次点击取消后立即出现需要重试的对话框，连接疑似被终止。

**已确认状态问题**：菜单每次打开调用 `getSessionToggleOption('block-input')`；Core `LoginConfigHandler::get_toggle_option()` 没有 `block-input` 分支，落入 `get_option('block-input')` 空值，因此会覆盖 UI/回调状态并显示未选中。

**待定位断线问题**：不要先假定目标端行为。必须分别在虚拟机与真机保存 `unblock-input` 点击前后的完整 bridge/Core 日志，按时间排序 `block-input-state`、`msgbox`、`session-error`、`session-closed`、retry dialog，确认命令、目标响应和 UI 重连判定哪一层出错。

**完成标准**：菜单状态只由官方 `update_block_input_state` 回调或等价权威状态驱动；开启与取消均真实生效、不会断线，重新打开菜单状态一致，断开后清零。

## 2026-06-20 三点菜单文件传输尚未端到端实现

**现象**：入口会请求文件权限并导航到 FileTransfer 页面，但用户实测功能不可用。

**原因边界**：历史上接入 `FileManager` 请求和 folder/job 事件只能证明 API 骨架存在，不能证明本地 URI 可读、远端目录返回、数据传输和确认流程可用。

**完成标准**：用真实文件逐项验证本地/远端目录、上传、下载、进度、覆盖/跳过、取消、错误恢复、完成校验和断线清理；权限拒绝与重新授权也必须可恢复。

## 2026-06-20 自动安装脚本选到不可安装的外部 HAP

**现象**：历史上 `AUTO_BUILD_INSTALL.bat --skip-build 192.168.11.102:36169` 选中 `99_Temp/harmonyos_build/.../entry-default-signed.hap`，设备报 `9568320 no signature file`；当时 `windows_hap/entry-default-signed.hap` 通过 `verify-app` 且直接安装成功。

**当前规则**：2026-06-21 16:26 清理后旧 `windows_hap` 目录已删除；当前标准 HAP 是 `99_Temp/harmonyos_build/11_Rustdesk_harmonyos/entry/build/default/outputs/default/entry-default-signed.hap`。若脚本再次生成 `windows_hap`，只能视为临时验包副本，不能长期保留。安装前必须对实际候选 HAP 做签名/双 ABI/native 依赖验证，不能只按文件名判断。
...
**现象**：`scripts\AUTO_BUILD_INSTALL.bat --skip-build 192.168.11.102:36169` 安装 HAP 成功，但 `aa start` 返回 `Error Code:10106102`，随后 `pidof com.open.rundesk` 无进程，hilog 中没有 `coreReady`、`session-connected` 或 `video-frame`。2026-06-14 core-73 和 core-74 复测时，`power-shell wakeup`、`uitest uiInput swipe` 和 `aa start -N` 都不能绕过锁屏，WindowManager 仍显示 `SCBScreenLock` 处于顶层。

**根因**：设备处于锁屏状态，系统阻止应用启动；这不是 HAP 构建、签名、安装或 native core 链接失败。

**解决**：先手动解锁设备，再重新执行安装/启动或单独执行 `hdc -t 192.168.11.102:36169 shell aa start -a EntryAbility -b com.open.rundesk`，随后重新抓取 hilog 验证 `coreReady=true`、`session-connected`、`peer-info`、`video-refresh-requested`、`video-frame`。不要把锁屏导致的无进程误判为 HAP、签名或 native core 链接失败。

**教训**：安装成功和启动成功必须分开记录；遇到 `10106102` 时不要继续分析视频流缺失，先处理设备解锁。

**2026-06-14 复核**：手机手动解锁后，`scripts\AUTO_BUILD_INSTALL.bat --skip-build 192.168.11.102:36169` 可同时完成安装和 `aa start`；本轮复核进程 `12565` 存活，`reports/hilog_latest_after_core74_post_docs_unlocked.txt` 记录 `coreReady= true` 7 次、`query-onlines-result` 14 次，app fatal/panic/signal 为 0。
...
**解决**：`AUTO_BUILD_INSTALL.bat` 对入参、USB 环境目标、无线目标和 fallback 列表都过滤包含 `[Empty]` 的值。无线复试使用 `scripts\AUTO_BUILD_INSTALL.bat --skip-build 192.168.11.102:36169`，安装成功并 `start ability successfully`。
...
**验证**: 2026-06-03 22:15 无线设备 `192.168.11.102:36169` 安装启动成功，hilog显示LAN发现正常工作，发现2个设备，无崩溃关键字，手动刷新无卡片消失。
...
**验证**: 2026-06-01 22:45，无线设备`192.168.11.102:36169`安装启动成功；日志显示`RustDesk bridge loader module registered (50 functions)`、`initializeRuntimeFn returned`为有效JSON、`Bootstrap snapshot`为`{"adapter":"official-native","coreReady":true,...}`
...
**解决**: 使用前先执行`hdc start`或确认DevEco已启动server；无线测试统一使用`hdc -t 192.168.11.102:36169 ...`；无线Offline时重新执行`hdc tconn 192.168.11.102:36169`；安装使用`hdc -t 192.168.11.102:36169 install -r <hap>`，权限单独处理
...
- 当前必须保留: `rustdesk_harmonyos_build/` 中的 SDK/HMS/DevEco mirror、vcpkg、external-src、tools、toolchains、patches，`rustdesk_harmonyos_signing/` 签名材料，`librustdesk_core/cargo_target/.../release/librustdesk_harmony_bridge.a` 当前双架构产物，以及两类 backups 最新 2 份。
- 已删除且不应再当作当前保留项: `rustdesk_harmonyos_build/build/`、`rustdesk_harmonyos_build/native_rust_core/target/`、`rustdesk_harmonyos_build/windows_hap/`、`rustdesk_harmonyos_build/rustdesk-1.4.7-clone/`、`%VSCODE_ROOT%/_tmp_rustdesk_1_4_7_src`。
...
**验证**: 旧规则下 `scripts\github_build_harmonyos.ps1 -ArtifactType both -VersionBump none` 通过；signed HAP 安装到 `192.168.11.102:36169` 成功；HAP native 库和签名校验通过；`scripts\audit_connection_chain.ps1` 为 `50 PASS, 0 FAIL, 0 SKIP`；`scripts\audit_full_function_rounds.ps1 -Rounds 100` 每轮 `96 PASS, 0 SKIP`。
...
## 2026-06-20 网络差时多次重试导致被控端产生一串访问请求

**现象**: 网络差时主控端发起多次重试，被控端收到同一 peer 的多个 PunchHole/RequestRelay/FetchLocalAddr 请求，创建多个并行连接。
**根因**: 1) OHOS 版 `rendezvous_mediator_ohos.rs` 完全缺失主版本的 `LAST_MSG`/`LAST_RELAY_MSG` 100ms 去重逻辑；2) 两个版本的 `server_ohos.rs` 都没有"同 peer 新连接取消旧连接"机制，`ALIVE_CONNS` 只记录 id，没有 peer→conn_id 映射。
**修复**: 1) `rendezvous_mediator_ohos.rs` 添加 `LAST_MSG`/`LAST_RELAY_MSG` 静态变量，handle_punch_hole/handle_request_relay/handle_intranet 中 100ms 内同地址消息直接跳过；2) `server_ohos.rs` 添加 `PEER_CONNS: Arc<Mutex<HashMap<String, i32>>>` 映射，LoginRequest 授权后查找同 peer 旧连接并发送 `close_reason: "Replaced by new connection"` 关闭，`cleanup_connection` 中 `retain(|_, v| *v != id)` 清理映射。
**验证**: core-32 CI 构建成功，虚拟机和真机安装验证 `coreReady=true`。
**教训**: ✅OHOS 版本必须与主版本保持去重逻辑一致；✅被控端应主动取消同 peer 旧连接，避免资源浪费和状态混乱。

## 2026-06-20 核心状态指示器滞后

**现象**: 核心状态指示器显示的 coreReady 状态有延迟，应用重启后可能显示过时状态。
**根因**: 1) `refreshTimer` 曾每 2000ms 才刷新一次；2) 后续为规避旧缓存，把 `EntryAbility` 刚取得的实时 bootstrap snapshot 也强制写成 `coreReady=false`，造成每次重装/启动先显示停止；3) ready 变化没有同步刷新 `coreModules`。
**修复**: 1) `refreshTimer` 保持 1000ms；2) `EntryAbility` 把实时 `snapshot.coreReady` 写入 AppStorage；3) `Index.refreshState/bridgeListener` 在 ready 变化时同步 AppStorage 并刷新模块信息；4) 核心状态短标签统一为“就绪/停止/未识”。
**验证**: v0.30.0 HAP 构建通过，待双设备安装复核。
**教训**: ✅持久化旧快照不能覆盖实时 snapshot；✅启动状态与模块详情必须由同一实时快照驱动。

## 2026-06-20 聊天 TAB 在 peer ID 变化时显示旧消息

**现象**: 当 activePeerId 变化（会话关闭后连接新 peer）时，聊天 TAB 仍显示旧 peer 的消息，或显示空内容。
**根因**: 1) 仅切换页面数组，没有删除旧 ID 的持久化消息；2) activePeerId 清空后曾回退到 recent/chat session，可能重新显示更旧 ID；3) 旧会话迟到事件可再次写回消息。
**修复**: `ChatService.retainOnlyPeer()` 在新目标确定时删除其他 ID 的 sessions/messages/local echoes 并立即持久化；迟到的旧 ID 消息直接丢弃；当前 peer 只允许 activePeerId 或 lastSessionPeerId，不再回退 recent/chat 列表；会话结束保留最新 ID 内容，新 ID 到来才清除旧内容。
**验证**: v0.30.0 HAP 构建通过，待双设备连接复核。
**教训**: ✅“只保留最新 ID”必须同时约束 UI、内存 Map、持久化数据和迟到事件，不能只换页面数组。

## 2026-06-20 远控质量菜单重复项和要求外字段

**现象**: 质量菜单中延迟和帧率与固定行重复显示，还出现了"目标"(target_bitrate)和"下载"(speed)等要求外字段。
**根因**: 质量菜单由固定行（Resolution/FPS/Latency/Speed/Connection/Zoom/Codec）+ 动态 `qualityMetricItems`（来自核心 quality-payload 事件）组成，动态项中的 delay/fps/speed/codec_format 与固定行重复，target_bitrate/send_speed/chroma 在要求外。
**修复**: 移除动态 `qualityMetricItems` 的 ForEach 渲染，只保留固定 7 项（尺寸/帧率/延迟/速度/连接/缩放/编码）。
**验证**: v0.30.0 构建验证通过。
**教训**: ✅质量菜单字段已写入 UI.md 规范，只允许 7 项，不得增减；✅核心事件中的重复数据不应在 UI 中重复显示。

## 2026-06-20 连接输入、终止状态和显示选项未形成闭环

**现象**: ID 输入只能输入数字且联想浮层出现时右侧按钮失效；连接结束后仍显示“连接已建立”；等待被控端确认时密码框会提前关闭；编码选择和默认显示/质量看似可选但实际不生效；服务器输入浮动标签使对话框跳高。
**根因**: 1) 输入 onChange 使用 `/[^\d]/g` 且连接校验统一要求长度 >= 9；联想 overlay 层级高于整个输入 Stack；2) 终止事件只改 core state，未统一清理 pending/password/status/monitor，`refresh()` 还会重新载入持久化旧快照；3) `msgbox success` 被误当成真实 connected；4) 编码写错为 `video-codec-preference`，核心真实键为 `codec-preference`，保存后也没有调用 `update_supported_decodings()`；5) 显示模式 state 没有参与 `resolveDisplayScale()`，设置页摘要行为不可点击；6) 服务器标签高度在 0/16 间变化。
**修复**: 输入支持数字 ID、IPv4、IPv6，纯数字才分组和联想，输入 Stack 提升到 overlay 之上；每次新连接先关闭旧请求，终止事件统一重置全部待连接 UI；connected 只接受 `session-connected/connection-ready/peer-info/video-frame`；会话与设置页共用可操作下拉项和自定义缩放滑块；核心持久化默认选项并在新 Session 注入，编码变化立即更新解码能力；服务器四行固定预留 68 高度。
**验证**: App HAP 构建通过；核心 arm64/x86_64 release 本地构建均通过；待双设备安装和运行态日志复核。
**教训**: ✅提示框成功不等于握手完成；✅官方 option 键必须从核心读取点反查；✅默认设置必须既持久化又在新会话创建时注入；✅浮动标签必须预留稳定行高。

## 2026-06-20 虚拟机无限重置与旧会话污染新连接（已修复）

**证据**: 旧安装包每约 150ms 重复产生同一批 session 事件；关闭旧会话并新建连接后，旧线程稍后上报 `Reset by the peer`，把已经建立的新会话改成错误。
**根因**: App 消费 native 事件时又调用 `markSessionConnected/markSessionError` 写回核心，形成反馈环；Rust handler 没有会话世代标识，旧线程回调仍能修改全局状态。
**修复**: App 移除事件回写；核心增加 `ACTIVE_SESSION_GENERATION`，新建和关闭会话都推进 generation，所有会修改会话状态或输出帧的 handler 回调先检查当前 generation。
**验证**: clean install 后 `coreReady=true` 首屏可见；连续关闭、重连、密码登录均未再出现旧连接迟到 reset，也没有重复事件风暴。

## 2026-06-20 密码首次连接失败、确认按钮被键盘遮挡（已修复）

**证据**: 旧逻辑点击确认后先出现 `session-closed`，约 200ms 后第二次 `connecting`，随后 `session-connected` 又立即 `Reset by the peer`；虚拟机布局同时显示确认按钮 `y=1809..1875`，软键盘从 `y=1757` 开始。
**根因**: 主动密码框的确认路径关闭当前空密码握手并立即新建密码连接，远端旧连接清理会重置新连接；确认按钮又落在软键盘命中区域内。
**修复**: 密码提交改为当前会话 `submitSessionPassword(password, rememberPassword)`，只在 native 会话确实不存在时才走重连 fallback；密码卡片根据 `avoidKeyboardHeight` 上移。
**验证**: 不记住密码和记住密码的首次连接均出现 `submitSessionPassword -> login -> session-connected -> connection-ready`，无 reset；记住密码后重启 App 可一次自动连接。
**教训**: ✅已有握手等待密码时应原会话补交凭据；✅不能用固定延时猜测 socket 已完全关闭；✅自动化点击前必须用 dumpLayout 对照键盘与按钮真实 bounds。

## 2026-06-20 ID 候选框抢占清除/连接点击（已修复）

**证据**: 虚拟机输入三位数字出现候选后，点击右侧区域会选中历史完整 ID 并启动连接。
**根因**: 候选列表通过整个连接面板的 `.overlay(...)` 挂载，zIndex 只改变绘制顺序，不能可靠限制覆盖层命中范围。
**修复**: 候选列表不再挂到覆盖整个面板的 `.overlay(...)`，而是作为外层 `Stack` 中的绝对定位悬浮层；它覆盖下方内容但不参与排版。宽度截到输入文本区，候选行和候选框显式使用 `HitTestMode.Block`，清除/连接命令区保持更高 zIndex 且位于候选命中矩形之外。
**验证**: 旧几何下候选显示时，点击清除后输入为空、候选消失、连接日志为空；点击连接只使用当前输入目标。最新完全悬浮且收紧命中区域的实现需在最终虚拟机/真机包再次回归。

## 2026-06-20 蜂窝网络直连局域网 IPv4 失败（预期行为）
...
## 2026-08-18 用户列表同步失败被误报为网络问题（已修复）

- **现象**：浏览器可以访问 API 地址，但客户端用户列表显示“同步失败，请检查 API 服务器网络”。
- **核对**：当前 API `http://liyan-fnosnas.dynv6.net:16888` 及 `/api/users` 路由可达；未携带令牌时按预期返回 HTTP 401，说明主机网络与路由正常，受保护请求仍可能因令牌或权限失败。

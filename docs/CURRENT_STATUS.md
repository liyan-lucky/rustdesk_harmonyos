...
- 登录成功时若服务器响应缺 `user` 字段，清空旧 user_info，避免新 token 与旧账户信息错配
- 密码弹窗重入防护：弹窗已显示时不再重置用户正在输入的密码，避免事件重复触发导致密码无法输入
- `shouldPromptForPassword` 收紧匹配条件，移除过宽的 `auth` 匹配

## 多显示器切换

- 被访问端有多个显示器时，显示菜单最上方提供对应序号的显示器按钮（含分辨率+选中态），点击切换
- 只有一个显示器时不显示显示器切换区块
- 消费 Rust 层 `displays` 事件（JSON 数组格式），调用 `sessionSwitchDisplay` 桥接函数切换
- 切换后画布/分辨率自适应复用现有 `display` 事件 + 帧驱动闭环

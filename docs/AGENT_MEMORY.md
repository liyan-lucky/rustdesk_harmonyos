...
| 2026-06-22 | ID 卡片菜单修改：菜单宽度从 248 缩至 210；"管理员"改为"终端（管理员 beta）"（i18n key `Terminal (Admin beta)`）；"始终通过中继连接"改为"中继连接"（i18n key `Relay connection`）；勾选图标从左侧移到右侧，使用 proicons 的 `CheckboxChecked`/`CheckboxUnchecked` SVG 替代不一致的 checkbox-outline.svg + Rect 空心方框，两状态图标大小完全一致；SVG 为 stroke 格式使用 colorFilter 着色。构建安装虚拟机验证通过。 |
| 2026-06-22 | 文件传输页面全面重构：删除旧布局（PaneSwitch/PathCard/Toolbar/CreateFolderCard/FileCard/TransferQueue），改为参考 RustDesk 官方文件管理器布局——顶部 logo 区域含关闭×、本地/远程切换按钮（选中蓝色）、三点菜单；下方工具栏含主页/后退/上一级/排序图标；文件列表为紧凑行（图标+名称+时间+三点菜单）；底部固定区域显示路径和总计（多选模式替换为操作栏：复制/删除/关闭；粘贴模式显示"粘贴到这里？"+来源信息+粘贴/关闭按钮）。新增功能：排序菜单（名称/类型/修改时间/大小）、三点菜单（刷新文件/多选/创建文件夹/显示隐藏文件）、文件项菜单（删除/多选/属性/重命名）、多选+复制→自动切换到对面→粘贴、隐藏文件过滤。新增 16 个 ft_*.svg 图标（stroke 格式+colorFilter 着色）。ArkTS 注意：Stack 无 justifyContent 属性，菜单弹窗用 Column 替代；ForEach 回调参数不能用对象字面量类型声明（arkts-no-obj-literals-as-types），改为独立 Builder 方法。 |
| 2026-06-22 | 新对话读取全部文档后构建安装验证：两个仓库 clean/ahead-behind=0，Core 最新 Release 仍为 `core-34`；使用 `build_harmonyos_hap.ps1` 增量构建 `0.33.6 / 1000182`，BuildInfo `2026-06-22 21:15`，bump=none；signed HAP `34,284,691` bytes / SHA256 `7760F8BBC1CC1B956D049A8DC4496DCAB35328CA60D999E73B30270BCABC9FB0`；验签/双 ABI/native 依赖检查通过；虚拟机 `127.0.0.1:5555` 安装启动成功，PID `12853`，`coreReady=true`，在线查询正常（`1283267036` 在线），fatal/panic/signal 为 0。 |
| 2026-06-22 | ID 卡片菜单"文件传输"和"终端(beta)"点击改为"开发中"提示：原代码直接 `navigateTo` 跳转页面，改为调用 `closePeerMenuAsDeveloping()` 弹出"功能开发中"Toast，与"查看摄像头"/"终端（管理员 beta）"行为一致；第三个参数 `false` 表示不显示右侧箭头图标。构建安装虚拟机验证通过（signed HAP `34,307,497` bytes）。 |
| 2026-06-22 | 终端页面重写（Terminal.ets）：删除旧布局（HistoryBar/SnippetList/OutputPanel/SessionBar/ExtraKeys/InputBar），改为新布局——顶部 header（关闭× + "终端" + 状态）→ 终端画面（黑色背景、等宽字体、可点击弹出键盘、长按可复制）→ 自定义键盘（Esc/Tab/Ctrl+C/方向键/Home/End/PgUp/PgDn 等 + 命令输入行）。构建安装虚拟机验证通过。 |
| 2026-06-23 | 文件传输/终端页面 UI 样式调整：1) Header 区域高度翻倍（56→112/48→96），padding 避让状态栏；2) 工具栏图标布局调整——主页图标在左，后退/上一级/排序图标移到右侧（Blank 分隔）；3) 底部栏三种模式（粘贴/多选/普通）都加了 `margin({ bottom: avoidNavigationBarHeight })` 避让导航栏；4) 长按选中——文件行添加 `LongPressGesture`，非多选模式长按自动进入多选并选中该项，多选模式下长按切换选中；5) 三点菜单/排序菜单/文件项菜单弹出位置修正为 `Stack({ alignContent: Alignment.TopEnd })` + `margin({ top: 160/200, right: 16 })`，靠近点击位置；6) 菜单面板背景改为 `theme_CARD_BG` + `opacity(0.92)` 半透明主题色，遮罩层 `#4D000000`；7) 页面区域扩展到屏幕边缘——移除 `expandSafeArea`，header 用 `avoidStatusBarHeight` padding 避让状态栏，底部用 `avoidNavigationBarHeight` margin 避让导航栏。 |
| 2026-06-23 | SVG 图标从 proicons 提取替换：所有 16 个 ft_*.svg + checkmark.svg + refresh.svg 从 proicons npm 包提取（HomeIcon/ArrowLeftIcon/ArrowUpIcon/ArrowSortIcon/FolderIcon/FileIcon/PhoneIcon/ComputerIcon/DeleteIcon/CopyIcon/ClipboardIcon/ArrowSyncIcon/EyeIcon/EyeOffIcon/AddSquareIcon/CheckmarkIcon），使用 Node.js 脚本解析 proicons JS 组件提取 SVG path/rect 元素属性，生成标准 stroke 格式 SVG（`stroke="#000000" fill="none"`）；修复重复属性问题（提取脚本生成重复 stroke/fill 属性，OHOS 渲染器解析失败导致图标不可见），使用属性去重脚本清理所有 SVG 文件；proicons 安装后卸载清理。 |
| 2026-06-23 | 排序图标替换为用户指定链接下载（proicons.com/icon/417686/download）：原 ArrowSortIcon 为上下箭头样式，替换为三横线排序样式（Material Design 风格），转换为 stroke 格式（`stroke="#000000" stroke-width="1.5" stroke-linecap="round"`），配合 `colorFilter(createStrokeIconColorFilter())` 主题匹配。 |
| 2026-06-23 | 文件传输页面布局修复：恢复为 Column 布局（header→toolbar→fileList→bottomBar），`List` 使用 `layoutWeight(1)` 撑满中间区域；之前尝试 Stack 布局（文件列表铺满全屏+header/bottomBar 叠加）导致底部全屏容器拦截滚动事件，`HitTestMode.Transparent` 只让点击穿透但滚动事件仍被拦截。 |
| 2026-06-23 | 终端页面布局同步修复：恢复为 Column 布局（header→terminalScreen→customKeyboard），终端画面 `padding({ top: 96 })` 避让 header；之前 Stack 布局同样有滚动拦截问题。 |
| 2026-06-23 | ID 卡片菜单"文件传输"恢复可点击进入页面：添加 `pendingNavigatePage` 状态变量，点击"文件传输"时设置 `pendingNavigatePage = 'pages/FileTransfer'` 然后调用 `handleConnect(id)`，连接成功后 `openRemoteControlForPendingSession` 根据 `pendingNavigatePage` 跳转到 FileTransfer 而非 RemoteControl，复用完整的连接流程（密码对话框、重试等）；`resetPendingConnectionState` 和 `resetConnectionUiAfterTerminalEvent` 中清除 `pendingNavigatePage`。"终端(beta)"保持直接 `navigateTo` 跳转（终端不需要先建立连接）。 |
| 2026-06-23 | 顶部渐变遮罩：文件传输页面添加 `linearGradient` 渐变遮罩层（暗色 `#171A22`/亮色 `#F0F4FA`），`hitTestBehavior(HitTestMode.Transparent)` 不阻挡交互；终端页面添加黑色渐变遮罩层。 |

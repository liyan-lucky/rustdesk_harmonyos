...
BUILD_DIR="$VSCODE_ROOT_LINUX/99_Temp/rustdesk_harmonyos_build"
mkdir -p "$BUILD_DIR"
cd "$BUILD_DIR"
...
cp "$RUSTDESK_HARMONY_BUILD_DIR/native_rust_core/target/harmony/librustdesk_harmony_bridge.a" \
   $VSCODE_ROOT_LINUX/11_Rustdesk_harmonyos/entry/src/main/libs/arm64/librustdesk_core.a
```
...
主要更新点：docs/CORE.md、docs/PROGRESS.md、docs/FILES.md、docs/ISSUES.md、docs/SESSION3_SUMMARY.md。
---
...
| `native_rust_core/src/bridge_api.rs` | peer option/info改走`harmony_bridge`，不直接访问`ui_interface/hbb_common` |
| `scripts/build_native_bridge.sh` | pkg-config/RUSTFLAGS/LIBCLANG_PATH/SODIUM target-specific变量 |

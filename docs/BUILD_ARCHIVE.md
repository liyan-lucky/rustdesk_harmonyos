...
- Linux 原生核心辅助脚本：`scripts/build_native_bridge.sh`（当前仅保留 `aarch64-unknown-linux-ohos`）
...
`build_hap.bat` 是增量 HAP 构建入口，不清缓存，但同样会通过标准 Hvigor wrapper 刷新 App 内构建时间。
...
- 旧的 `INSTALL_HAP.bat`、中文固定 SDK batch、`install_with_core.sh`、`build_native_module.sh`、OpenHarmony test-sign/repack/install shell 链路，因为它们绕过当前标准 HAP 构建或依赖历史输出路径。
- 非当前 ABI 的 armv7/x86_64 OHOS wrapper，因为当前 HAP 只打包 `arm64-v8a`。

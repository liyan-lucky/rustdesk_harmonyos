...
- 平移回弹代码逻辑验证通过（uitest 不支持多指触控，无法自动化测试 2指/3指平移）
- 远程会话实测：显示菜单"原始尺寸"模式可正常切换，画面超出边界
- commit `176674d` 已推送
- 待触发 Release workflow 发布正式版
...
# 2026-08-18 登录信息缓存与同步

- 登录提供商和账户用户列表已持久缓存，切换页面不再强制等待服务器。
- 登录页、登录弹窗和账户页已增加手动同步，并在页面停留期间按 60 秒周期自动刷新。
- 缓存按 API/账户隔离，防止官方、自建服务器和不同用户之间数据串用。

## 2026-08-18 发布状态收口

- 用户列表同步增加 HTTP 状态和响应格式诊断，不再把授权或服务端错误误报成网络问题。

...
> 本节是当前权威快照；下方按日期排列的旧版本、旧 Release、旧设备和旧哈希仅用于历史追溯。

- App：`0.34.34` / `versionCode 1000279`，BuildInfo `2026-08-18 22:29`。
- arm64 Core：`133353216` bytes，SHA256 `AE629ED2045851469952DAF881D86E963C5344C787745AA5DDB69387EBBA41B5`，编译时间 `2026-08-18 09:14`。
- x86_64 Core：`130840066` bytes，SHA256 `2199D151CD6C2900BD1FB489561F6C7716314147DF2F87F542178134C4482476`，编译时间 `2026-08-18 22:27`。
- 双 ABI 签名 HAP 构建通过；arm64 HAP 已覆盖安装到 `192.168.0.106:36169`。
- 设备锁屏时 `hdc install` 可成功，但 `aa start` 会被系统以 `10106102` 拒绝；这属于启动验证受阻，不是安装失败。
- 主分支为 `master`，仅保留 `backup` 快照分支。

...
1. **默认图标颜色改为灰色**（`#667085`）：`iconColorMode` 默认值从 `'colorful'` 改为 `'#667085'`，首次安装时所有图标灰色显示
2. **TAB 选中/未选中颜色区分**：灰色模式下选中用主题蓝色 `#0071FF`，未选中用灰色 `#667085`
3. **开关颜色跟随图标颜色**：新增 `resolveToggleColor(icon)` 方法，多彩模式下开关颜色匹配图标颜色，灰色模式下统一灰色
4. **Toast 颜色响应式**：修复 `resolveIconFillColor` 非响应式问题，改用 `@StorageLink`/`@StorageProp` 传入 `iconColorMode`
5. **"屏幕常亮"重命名**：从"调试时保持亮屏"改为"屏幕常亮"，默认关闭
6. **构建验证**：版本 `0.35.23 (1000322)`，构建通过，HAP 已安装到设备 `2NX0224429035123`
...
1. **版本与分支**：当前版本 `0.35.23 (1000322)`；所有修改直接进入 `master`，仅保留 `backup` 快照分支。

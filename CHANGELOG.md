...
- 自建 API 的第三方登录项严格跟随服务器配置，OAuth 授权事务按 API 服务器隔离。
- 登录提供商和账户用户列表按 API/账户缓存，支持手动同步及页面内 60 秒自动刷新。
- 通讯录对齐个人地址簿 API，支持分页设备、彩色标签、标签管理、设备增删改、标签分配和过滤，并保留旧协议回退。
- IP 地址设备增加直连在线探测和持续轮询；共享页密码设置接入核心真实配置接口。
- 会话自定义键盘、触摸隔离、沉浸式状态栏、主题工具栏和 App 调试信息输出得到完善。

### 修复

- 用户列表同步失败不再统一误报为网络问题，现区分授权失效、权限不足、服务端错误、响应格式异常和网络不可达。
- 登录与账户同步按钮统一放在搜索按钮之后，并与通讯录使用相同主题图标和旋转动画。
- 修复通讯录本地新增未上传、标签切换刷新延迟、菜单状态延迟和共享密码菜单无效等问题。

### 构建与分支

- 当前版本 `0.34.34 (1000279)`；arm64/x86_64 签名 HAP 构建通过，arm64 包已安装到 `192.168.0.106:36169`。
- `master` 为唯一开发主分支，仅保留 `backup` 快照分支。
